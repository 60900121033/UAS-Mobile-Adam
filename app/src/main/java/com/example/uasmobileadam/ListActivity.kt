package com.example.uasmobileadam

import android.content.Intent
import android.os.Bundle
import androidx.appcompat.app.AppCompatActivity
import com.example.uasmobileadam.databinding.ActivityListBinding
...
class ListActivity : AppCompatActivity() {
    ...
    private fun getList():ArrayList<Video>{
        val dataThumbnail=resources.obtainTypedArray(R.array.video_thumbnail)
        val dataName=resources.getStringArray(R.array.video_title)
        val dataDescription=resources.getStringArray(R.array.video_description)
        ...
        for (i in dataName.indices){
            val video=Video(dataThumbnail.getResourceId(i,-1),dataName[i],dataDescription[i],videoId.getResourceId(i,-1),dataSoundNames[i])
            listvideo.add(video)
        }
        return listvideo
